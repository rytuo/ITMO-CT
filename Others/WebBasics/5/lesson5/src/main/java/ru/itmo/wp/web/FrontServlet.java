...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        String lang = updateLanguage(request);
        ...
        try {
            ...
            try {
                ...
                throw new ServletException("Can't render template [pageClass=" + pageClass + ", action=" + method + "]", exception);
            }
        }
    ...
    private String updateLanguage(HttpServletRequest request) {
    ...
    private void invokeMethod(Method method, Object page, HttpServletRequest request, Map<String, Object> view) throws InvocationTargetException, IllegalAccessException {
        Object[] passedParameters = Arrays.stream(method.getParameterTypes())
                .map(expectedParameter -> expectedParameter == Map.class ? view : request)
                .toArray();

        method.invoke(page, passedParameters);
    }

    private Method findMethod(Class<?> currentClass, String methodName) {
        Method method = null;
        for (Class<?> clazz = currentClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            Optional<Method> validMethod = Arrays.stream(clazz.getDeclaredMethods())
                    .filter(parameters -> parameters.getName().equals(methodName))
                    .filter(parameters -> Arrays.stream(parameters.getParameterTypes()).allMatch(parameterType ->
                            (parameterType == Map.class || parameterType == HttpServletRequest.class)))
                    .findFirst();
            if (validMethod.isPresent()) {
